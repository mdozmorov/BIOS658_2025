---
title: "Gene Set Testing with topGO"
...
es$condition <- ifelse(grepl("none", es$condition), "control", "dex4")
es$condition <- factor(es$condition, levels=c("control", "dex4"))
```
...
r2tab <- AnnotationDbi::select(GO.db, keys=rownames(r2)[1:10],
...
```{r}
r2tab <- AnnotationDbi::select(GO.db, keys=rownames(r2)[r2$Direction == "Down"][1:5],
                columns=c("GOID","TERM","DEFINITION"), 
                keytype="GOID")
r2tab[,1:2]
```
